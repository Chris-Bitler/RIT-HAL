...
describe("Emoji Util tests", () => {
    ...
    test("should return png if gif errors and png doesn't", async () => {
        when(mockGet).calledWith("https://cdn.discordapp.com/emojis/1234.gif").mockRejectedValue(false);
        when(mockGet).calledWith("https://cdn.discordapp.com/emojis/1234.png").mockResolvedValue(true);
        const value = await getEmojiExtension("1234");
        expect(value).toEqual("https://cdn.discordapp.com/emojis/1234.png");
    });
    test("should reject if gif errors and png also does", async () => {
        when(mockGet).calledWith("https://cdn.discordapp.com/emojis/1234.gif").mockRejectedValue(false);
        when(mockGet).calledWith("https://cdn.discordapp.com/emojis/1234.png").mockRejectedValue(false);
        try {
