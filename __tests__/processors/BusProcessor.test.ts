...
import {Arrival, ArrivalResponseContainer, BusRoute, Stop} from "../../src/types/Bus";
...
describe("Bus Processor tests", () => {
    ...
    beforeEach(() => {
        ...
        when(mockGet).calledWith("/routes.json").mockResolvedValue({
            data: {
                data: {
                   "643": [route]
                }
        ...
        when(mockGet).calledWith("/stops.json").mockResolvedValue({
            data: {
                data: [
                    stop
                ]
        ...
        when(mockGet).calledWith("/arrival-estimates.json").mockResolvedValue({
            data: {
                data: [
                    arrivalContainer
                ]
