...
export class StarboardProcessor {
    ...
    async handleRemovedMessage(message: Message | PartialMessage) {
        ...
        if (wasDeletedViaUnstar) {
            this.deletedViaUnstar = this.deletedViaUnstar.filter(
                (deletedMsg) => {
        ...
        if (channel instanceof TextChannel) {
            const starboardChannelId = await this.fetchStarboardChannel(
                channel.guild.id
            );
            if (starboardChannelId) {
                const starboardChannel = channel.guild.channels.resolve(
                    starboardChannelId
                );
                if (starboardChannel instanceof TextChannel) {
                    const starboardedMessage = this.messages.find(
                        (msg) => msg.originalMessageId === message.id
                    );
                    const starboardMesssage = this.messages.find(
                        (msg) => msg.starboardMessageId === message.id
                    );
                    if (starboardedMessage) {
                        const messageToRemove = await safelyFetchMessage(
                            starboardChannel,
                            starboardedMessage.starboardMessageId
                        );
                        if (messageToRemove) {
                        ...
                        const message = await safelyFetchMessage(
                            starboardChannel,
                            starboardMesssage.starboardMessageId
                        );
                        if (message) {
    ...
    async respondToStarReaction(user: User, messageReaction: MessageReaction) {
        ...
        if (channel instanceof TextChannel && reaction.emoji.name === '⭐') {
            ...
            if (requiredAmount && starboardChannelId) {
                ...
                const existingStarboard = this.messages.find(
                    (message) =>
                        message.originalMessageId === reaction.message.id
                );
                ...
                if (starboardChannel instanceof TextChannel) {
                    if (
                        reaction.count &&
                        reaction.count >= Number(requiredAmount)
                    ) {
                        if (existingStarboard) {
                            await this.updateInStarboard(
                                reaction,
                                starboardChannel,
                                channel,
                                existingStarboard
                        ...
                        const starboardMessage = await safelyFetchMessage(
                            starboardChannel,
                            existingStarboard.starboardMessageId
                        );
                        if (starboardMessage) {
                            this.deletedViaUnstar.push(existingStarboard);
                            this.messages = this.messages.filter(
                                (message) =>
                                    message.starboardMessageId !==
                                    starboardMessage.id
                            );
    ...
    async addToStarboard(
        ...
        if (author) {
            ...
            embed.addField(
                'Stars',
                `${reaction.count} ⭐ <#${originalChannel.id}>`
            );
     ...
     */
    async updateInStarboard(
        reaction: MessageReaction,
        starboardChannel: TextChannel,
        originalChannel: TextChannel,
        starboardedMessage: StarboardedMessage
    ) {
        const starboardMessage = await safelyFetchMessage(
            starboardChannel,
            starboardedMessage.starboardMessageId
     ...
     */
    async fetchStarboardRequiredCount(
        serverId: string
    ): Promise<string | null> {
