...
export class ModProcessor {
     ...
     */
    async muteUser(memberToMute: GuildMember, muter: GuildMember, reason: string, expiration: number): Promise<void> {
        ...
        if (mutedRoleId) {
            ...
            const existingMute = this.mutes.filter((mute: Mute) => mute.memberId === memberToMute.id);

            if (existingMute) {
                this.mutes = this.mutes.filter((mute) => mute.memberId !== memberToMute.id);
            }
     ...
     */
    async banUser(memberToBan: GuildMember, banner: GuildMember, reason: string, expiration: number): Promise<void> {
        const expirationDateTime = Date.now() + expiration;
        const existingBan = this.bans.filter((ban) => ban.memberId === memberToBan.id);
     ...
     */
    async unbanUser(guild: Guild, user: User|string, automatic = false): Promise<void> {
    ...
    async tickPunishments(client: Client): Promise<void> {
        const expiredMutes = this.mutes.filter((mute) => Date.now() > mute.expiration);
        const expiredBans = this.bans.filter((ban) => Date.now() > ban.expiration);
     ...
     */
    async fetchPunishments(serverId: string, memberId: string): Promise<Punishment[]> {
    ...
    async fetchMutedRoleId(serverId: string): Promise<string|null> {
        const mutedRoleId = await ConfigProperty.getServerProperty("muted.role", serverId);
