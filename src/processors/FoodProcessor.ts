...
export async function getOpenPlaces(): Promise<FoodPlace[]|null> {
    try {
        ...
        const response = await axios.get("https://www.rit.edu/fa/diningservices/places-to-eat/hours");
...
export async function getSpecials() {
    ...
    try {
        const response = await axios.get("https://www.rit.edu/fa/diningservices/daily-specials");
        const data = response.data;
        const $ = cheerio.load(data);
...
function addSpecialsSections(place: PlaceMenu, embed: MessageEmbed): MessageEmbed {
    if (place.breakfast) {
...
async function fetchLastDailyInfo(serverId: string): Promise<DailyPostInfo> {
    const lastTime = await ConfigProperty.getServerProperty("food.last", serverId);
    const channel = await ConfigProperty.getServerProperty("food.channel", serverId);
    const enabled = await ConfigProperty.getServerProperty("food.enabled", serverId);
