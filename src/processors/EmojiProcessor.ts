...
export class EmojiProcessor {
    ...
    allowedMemoized = memoize(this.allowedChannels, {maxAge: 1000*60*30, promise: true, preFetch: true});
    regex = /<.?(:.+?:)[0-9]+?>/g
    ...
    countEmojis = (message: string): Map<string, number> => {
        const matches: IterableIterator<RegExpMatchArray> = message.matchAll(this.regex);
