...
import {CONFIG_SET, NAME_TAKEN_ERR, setMailConfig} from "../processors/MailProcessor";
    ...
    async useCommand(client: Client, evt: Message, args: string[]): Promise<void> {
        ...
        if (serverName && channel && serverId) {
            ...
            if (result === CONFIG_SET) {
                await evt.channel.send(getInformationalEmbed(
                    "Mail config updated",
                    `Server admin mail configuration updated`
                ));
            } else if (result === NAME_TAKEN_ERR) {
                await evt.channel.send(getErrorEmbed(
