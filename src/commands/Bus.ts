...
export class Bus extends Command {
  ...
  async useCommand(
    ...
    const sender = (evt.channel as TextChannel).guild.members.resolve(
      evt.author.id
    );
    if (sender) {
      if (args.length < 1) {
  ...
  async forceRefresh(channel: TextChannel, hasPerm: boolean): Promise<void> {
    if (hasPerm) {
      ...
      await channel.send(
        getInformationalEmbed("Refreshed", "Forced refresh of bus information")
      );
    }
  }
  ...
  async showStops(evt: Message, args: string[]): Promise<void> {
    ...
    if (route) {
      try {
        const arrivals: ArrivalTimes = await this.busProcessor.getArrivalTimes(
          route
        );
  ...
  getArrivalsEmbed(routeName: string, arrivals: ArrivalTimes): MessageEmbed {
    const embed = new MessageEmbed().setTitle(`${routeName} upcoming stops`);
