...
export class Mod extends Command {
        ...
        if (args && args[0] !== undefined && evt.member) {
            ...
            switch(args[0].toLowerCase()) {
                ...
                case "kick":
                    ...
                    break;
                case "actions":
                    await this.listActions(channel, sender, args.length >= 2 ? args[1] : "N/A");
                    break;
                default:
                    break;
            }
    ...
    parseArgsTime(evt: Message, args: string[], type: string): CommandArgsTime {
        ...
        if (args.length >= 2) {
            const firstMention: GuildMember|undefined = evt.mentions.members?.first();
            if (firstMention) {
     ...
     */
    async muteUser(channel: TextChannel, sender: GuildMember, cmdArgs: CommandArgsTime): Promise<void> {
        if (cmdArgs.target && cmdArgs.time && cmdArgs.reason) {
            await modProcessor.muteUser(cmdArgs.target, sender, cmdArgs.reason, cmdArgs.time);
     ...
     */
    async banUser(channel: TextChannel, sender: GuildMember, cmdArgs: CommandArgsTime): Promise<void> {
        if (cmdArgs.target && cmdArgs.time && cmdArgs.reason) {
            const expirationDate = moment(Date.now() + cmdArgs.time);
            const expirationDateString = moment.tz(expirationDate, "America/New_York").format("MMMM Do YYYY, h:mm:ss a");
     ...
     */
    async listActions(channel: TextChannel, sender: GuildMember, name: string): Promise<void> {
        const members = channel.guild.members.cache.filter((member) => member.user.username.toLowerCase().includes(name.toLowerCase()));
        const member = members.first();
        if (member) {
            const id = member.id;
            const punishments: Punishment[] = await modProcessor.fetchPunishments(channel.guild.id, id);
                const embed = new MessageEmbed().setTitle(`Found punishments - ${member.nickname || member.displayName}`);
                punishments.forEach((punishment) => {
                    const expirationDateString = moment.tz(punishment.expiration, "America/New_York").format("MMMM Do YYYY, h:mm:ss a");
                    embed.addField(
                        `Punishment`,
                        `Name: ${punishment.userName}\n` +
                        `Type: ${punishment.type}\n` +
                        `Reason: ${punishment.reason}\n` +
                        `Expiration: ${punishment.expiration ? expirationDateString : "N/A"}\n` +
