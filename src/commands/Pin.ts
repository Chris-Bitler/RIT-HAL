import {Client, Guild, Message, MessageEmbed, Permissions, TextChannel} from "discord.js";
...
export class Pin extends Command {
  async useCommand(client: Client, evt: Message, args: string[]): Promise<void> {
    if (args && args[0] !== undefined && evt.channel instanceof TextChannel) {
      ...
      if (urlParts.length >= 2) {
        ...
        try {
          ...
          const starboardChannelId = await this.fetchStarboardChannelId(guild.id);
          if (starboardChannelId) {
            const messageChannel = guild.channels.resolve(channelId);
            const starChannel = guild.channels.resolve(starboardChannelId);
            if (messageChannel  && messageChannel instanceof TextChannel && starChannel && starChannel instanceof TextChannel) {
              await this.getAndSendEmbed(guild, evt, messageChannel, starChannel, messageId, args[0]);
            } else {
        ...
        await evt.channel.send(getErrorEmbed("Please use a valid discord message url"));
      }
    }
  }
      ...
      if (messageMember && pinningMember) {
        ...
        await evt.channel.send(
            getInformationalEmbed(
                "Pinned",
                "Message from " + messageMember.displayName + " pinned in starboard."
            )
        );
      } else {
        await evt.channel.send(getErrorEmbed("Cannot resolve member who posted the message to pin"));
      }
    } else {
      await evt.channel.send(getErrorEmbed("Cannot resolve message to pin"));
    }
  }
  ...
  async fetchStarboardChannelId(serverId: string): Promise<string|null> {
    const starboardId = await ConfigProperty.getServerProperty("starboard.channel", serverId);
