            ...
            if (!inCooldown) {
                ...
                if (mentioned) {
                    ...
                    embed.setDescription(
                        `${evt.member} has thrown a snowball at ${mentioned}! :snowflake: :white_circle:`
                    );
                    await evt.channel.send(embed);
                    setCooldown(
                        COOLDOWN_TYPE.SNOWBALL,
                        evt.member,
                        Date.now() + COOLDOWN
                    );
                } else {
                    evt.channel.send(
                        getErrorEmbed(
                            'You need to specify someone to throw a snowball at.'
                        )
                    );
                }
            } else {
                ...
                if (cooldown) {
                    ...
                    evt.channel.send(
                        getErrorEmbed(
                            `You threw a snowball recently. Try again in about ${minutes} minutes`
                        )
                    );
                }
            }
