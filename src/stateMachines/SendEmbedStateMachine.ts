...
const setupStep: Step<SendEmbedContext, Result> = (context: SendEmbedContext) => {
    context.lastMessage.channel.send("What do you want the embed title to be? (Say stop at any time to exit this process)");
    return [{}, parseTitleStep];
};

// Parse the title input and ask for description
const parseTitleStep: Step<SendEmbedContext, Result> = (context: SendEmbedContext) => {
...
const parseDescriptionStep: Step<SendEmbedContext, Result> = ((context, result) => {
    context.lastMessage.channel.send("Do you want to send an image? (no for no image)");
    return [{
        title: result?.title,
...
const parseChannelAndSendStep: Step<SendEmbedContext, Result> = ((context, result) => {
    const channel = context.lastMessage.content.replace(/[#<>]+/g, "");
    const embed = new MessageEmbed()
        .setTitle(result?.title)
        .setDescription(result?.description)
        .setImage(result?.imageUrl ? result?.imageUrl : "");
    (context.lastMessage.guild?.channels.resolve(channel) as TextChannel).send(embed);
    return [result ?? {}];
});
...
export class SendEmbedStateMachine {
    ...
    private steps: Map<string, [Result, Step<SendEmbedContext, Result>]> = new Map();
    ...
    handleStep(client: Client, message: Message): void {
        ...
        if (message.content.toLowerCase() !== "stop") {
            if (step) {
                ...
                if (futureStep) {
                    this.steps.set(message.author.id, [nextStepResult, futureStep]);
                } else {
